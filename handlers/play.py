...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    draw.text((205, 670),
        f"Permintaan Oleh: {requested_by}",
        (255, 255, 255),
        font=font,
    )
    img.save("final.png")
...
def r_ply(type_):
    ...
    mar = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton('Daftar Music 📖', 'playlist'),
...
async def p_cb(b, cb):
    ...
    if type_ == 'playlist':           
        ...
        if not queue:   
            await cb.message.edit('Tidak ada player')
        temp = []
        for t in queue:
            temp.append(t)
        now_playing = temp[0][0]
                      ...
                      if administrator == message.from_user.id:  
                          ...
                          try:
                              ...
                              await lel.edit(
                                  ...
                                  "\n\nAtau tambahkan @PacarFerdilla ke Grup Anda dan coba lagi</b>",
                              )
                              pass
    try:
        chatdetails = await USER.get_chat(chid)
        ...
        await lel.edit(
            f"<i> {user.first_name} Userbot tidak ada dalam grup ini, Minta admin untuk tambahkan Assistant {user.first_name} secara manual</i>"
        )
        return     
...
async def jiosaavn(client: Client, message_: Message):
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        m = await client.send_photo(
            ...
            reply_markup=keyboard,
            photo="final.png",
            caption=f"#️⃣ Lagu yang Anda minta **Sedang Antri** di posisi {position}",
